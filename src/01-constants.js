...
/* ============================================================
  ...
  =============================================================== */
    const DataLaVista = {
      ...
      FIELD_TYPE_ICONS: {
      'text': { icon: 'T', cls: 'type-text' },
      'number': { icon: '#', cls: 'type-number' },
    ...
    TABLE_SOURCE_ICONS: {
      ...
      // types: 'all' = works on any, 'numeric' = numbers only, 'ordered' = numbers+dates
      QB_AGGS: [
      ...
      /* ===== STATE ===== */
      state: {
        source: { type: 'sharepoint', url: '', auth: 'current', token: '' },
       ...
       */
      safeDragSet: function (event, obj) {
       ...
       */
      safeJSONParse: function (str, label) {
        try {
    ...
    /* ============================================================
    Cyberdyne Pipeline
    [Come with me if you want to load...]
    =============================================================== */
    const CyberdynePipeline = {
      ...
      AGG_OPTIONS: [
        '', 'COUNT', 'COUNT(DISTINCT)', 'SUM', 'AVG', 'MEDIAN', 'MIN', 'MAX',
        'FIRST', 'LAST', 'STDEV', 'VAR'
      ],
    ...
    const state = {
      ...
      tables: {},        // internalName (DSName_Table) -> { displayName, alias, fields: [], data: [], loaded: false, dataSource: 'DSName' }
      queryMode: 'basic',
      basicQB: { tableName: null, selectedFields: [], fieldAggs: {}, conditions: [], sorts: [], groupBy: [], rowLimit: 500 },
      advancedQB: { nodes: {}, joins: [], activeJoinIdx: -1 }, // for advanced mode
      sql: '',
      sqlLocked: false,
      queryResults: null,  // array of row objects
      queryColumns: [],
    ...
    // Table/source type icons — SharePoint has sub-keys (list vs library); others are flat.
    const TABLE_SOURCE_ICONS = {
      sharepoint: {
        list:    { icon: '📋', title: 'SharePoint List' },
        library: { icon: '📁', title: 'Document Library' }
      },
      json:    { icon: '🗂️', title: 'JSON' },
      csv:     { icon: '📊', title: 'Spreadsheet' },
      excel:   { icon: '📗', title: 'Excel' },
      default: { icon: '⊞', title: 'Table' }
    };

    /** Returns the TABLE_SOURCE_ICONS entry for a table object from state.tables. */
    function getTableIcon(t) {
      if (t.sourceType === 'sharepoint') {
        return TABLE_SOURCE_ICONS.sharepoint[t.isDocLib ? 'library' : 'list'];
      }
      return TABLE_SOURCE_ICONS[t.sourceType] || TABLE_SOURCE_ICONS.default;
    }
    ...
    const DATE_MACRO_VALS = new Set(DATE_MACRO_OPS.map(o => o.val));
